...
if (track) {
  ...
  var nextButton = document.querySelector(".carouselButtonRight");
  var prevButton = document.querySelector(".carouselButtonLeft");
  ...
  prevButton.addEventListener("click", function () {
    ...
    var currentDot = dotsNav.querySelector(".currentSlide");
    var index = slides.findIndex(function (slide) {
      return slide === currentSlide;
    });

    if (index === 0) {
      track.style.transform = "translateX(-".concat(slides[slides.length - 1], ")");
      var lastSlide = slides[slides.length - 1];
      var lastDot = dotsNav.lastElementChild;
  ...
  nextButton.addEventListener("click", function () {
    ...
    if (index === slides.length - 1) {
      ...
      var firstDot = dotsNav.firstElementChild;
...
var sendLetter = document.querySelector("#sendLetter");
...
function handleIntersectionFadeInLeftForEach(entries) {
  entries.map(function (entry, index) {
    if (entry.isIntersecting) {
      ...
      entry.target.style.animationDelay = "".concat(index / 5, "s");
    }
  });
}

var observerFadeInLeftForEach = new IntersectionObserver(handleIntersectionFadeInLeftForEach); // Accordion animation

var accordion = document.querySelectorAll(".box");
...
}).resize(); // inizio codice dark theme
...
var spinner = document.querySelector('#spinner'); // Spinner
...
animatedSpinner(); //Get the button:
